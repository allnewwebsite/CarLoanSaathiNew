...
import { DEAD_CASE_REASONS } from "./financeLeadPage.helpers.js";
...
function DeadCaseActionButton({ lead, onMoved }) {
  ...
  const submit = async () => {
    if (!reason || !notes.trim()) {
      setError("Select a reason and write notes before moving this case.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      await api.post(`/dealer/leads/${lead.id}/dead-case`, { reason, notes });
      setOpen(false);
      onMoved?.();
    } catch (requestError) {
      setError(requestError.response?.data?.message || "Could not move this case.");
    } finally {
      setSaving(false);
    }
  };
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="rounded-md border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-medium text-red-700 hover:bg-red-100">Move To Dead Case</button>
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <button type="button" onClick={() => setOpen(false)} className="rounded-md p-1 text-slate-500 hover:bg-slate-100" aria-label="Close">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-3 p-4">
              <label className="block text-sm font-medium text-slate-700">
                Dead Reason *
                <select value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#0d47a1] focus:ring-2 focus:ring-blue-100">
                  <option value="">Select reason</option>
                  {DEAD_CASE_REASONS.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </label>
              <label className="block text-sm font-medium text-slate-700">
                Dead Notes *
                <textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} className="mt-1.5 w-full resize-none rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-[#0d47a1] focus:ring-2 focus:ring-blue-100" placeholder="Write why this case should leave the active workflow." />
              </label>
              {error ? <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p> : null}
            </div>
            <div className="flex justify-end gap-2 border-t border-slate-200 px-4 py-3">
              <button type="button" onClick={() => setOpen(false)} className="h-9 rounded-md border border-slate-200 px-3 text-xs font-medium text-slate-700">Cancel</button>
              <button type="button" onClick={submit} disabled={saving} className="h-9 rounded-md bg-[#0d47a1] px-3 text-xs font-medium text-white disabled:opacity-50">{saving ? "Saving..." : "Confirm"}</button>
            </div>
          </div>
        </div>
      ) : null}
...
function LeadActions({ lead, onMoved }) {
  return <div className="flex flex-wrap gap-2"><DocumentsButton lead={lead} /><DeadCaseActionButton lead={lead} onMoved={onMoved} /></div>;
}

function leadRows(leads, mode = "total", onMoved) {
  return leads.map((lead) => {
    ...
    if (mode === "status") {
      ...
      cells.push(<LeadActions key="actions" lead={lead} onMoved={onMoved} />);
    ...
    if (mode === "cases") {
      return {
        ...
        cells: [
          ...
          <StatusBadge key="status" lead={lead} />,
          dateTime(lead.statusUpdatedAt || lead.updatedAt || lead.createdAt),
          <LeadActions key="actions" lead={lead} onMoved={onMoved} />,
        ],
      };
    }
    return {
...
export function TotalLeadsScreen() {
  ...
  const refreshAfterDeadCase = () => loadLeads({ page, silent: true });
  return (
    <div className="space-y-4">
      <SectionTitle title="Total Leads" subtitle="All cases submitted by this dealership finance desk." />
      <Table headers={["Case ID", "Customer Name", "Mobile Number", "Customer City", "Assigned Bank", "Loan Amount", LEAD_TABLE_LABELS.generatedDate, "Finance Manager", LEAD_TABLE_LABELS.currentStatus, LEAD_TABLE_LABELS.assignedExecutive, LEAD_TABLE_LABELS.executiveMobile, "Actions"]} rows={leadRows(leads, "total", refreshAfterDeadCase)} loading={loading} page={page} total={total} hasMore={hasMore} onPage={pageTo} />
    </div>
  );
}
...
export function StatusScreen() {
  ...
  const refreshAfterDeadCase = () => loadLeads({ page, status, silent: true });
