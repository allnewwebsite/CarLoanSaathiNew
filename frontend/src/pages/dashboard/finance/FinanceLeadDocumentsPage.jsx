    ...
    <section className="space-y-5">
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="flex flex-col gap-1 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-base font-semibold text-slate-900">Customer Uploaded Documents</h2><p className="mt-1 text-xs text-slate-500">Upload any customer document. Request badges only indicate documents the Loan Executive still needs.</p></div>
              ...
              {uploadTypes.map((type) => {
                ...
                return (
                  <tr key={type} className={isRequested ? "bg-amber-50/40" : "bg-white"}>
                    <td className="min-w-40 px-4 py-3"><p className="font-medium text-slate-900">{type}</p>{isRequested ? <span className="mt-1 inline-flex rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-700">Requested</span> : <span className="mt-1 inline-flex rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold text-slate-500">Not Requested</span>}</td>
