          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {uploadTypes.map((type) => {
                ...
                const canUpload = !requestedTypes.length || isRequested;
                return (
                  <tr key={type} className={isRequested ? "bg-amber-50/40" : "bg-white"}>
                    ...
                    <td className="min-w-56 px-4 py-3"><label className={`flex h-9 items-center justify-between gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs text-slate-600 ${canUpload ? "cursor-pointer" : "cursor-not-allowed opacity-50"}`}><span className="max-w-36 truncate">{canUpload ? file?.name || "Choose file" : "Not requested"}</span>{file ? <button type="button" onClick={(event) => { event.preventDefault(); setFiles((current) => ({ ...current, [type]: null })); }} className="text-slate-400 hover:text-slate-700" aria-label={`Remove ${type} file`}><X className="h-3.5 w-3.5" /></button> : null}<input type="file" disabled={!canUpload} className="hidden" accept=".pdf,image/png,image/jpeg" onChange={(event) => setFiles((current) => ({ ...current, [type]: event.target.files?.[0] || null }))} /></label>{percent > 0 ? <div className="mt-2 h-1 rounded-full bg-slate-100"><div className={`h-1 rounded-full ${percent < 0 ? "bg-red-500" : "bg-[#0d47a1]"}`} style={{ width: `${Math.max(percent, 8)}%` }} /></div> : null}</td>
                    <td className="px-4 py-3"><button type="button" onClick={() => upload(type)} disabled={!file || !canUpload} className="h-9 whitespace-nowrap rounded-md bg-[#0d47a1] px-3 text-xs font-semibold text-white disabled:opacity-40">{document ? "Replace" : percent < 0 ? "Retry" : "Upload"}</button></td>
